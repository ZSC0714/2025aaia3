...
class Solution {
public:
    ListNode* myReverse(ListNode* head) { // 自訂函釋(剛刷過week14-2.cpp)
        if(head==nullptr || head->next==nullptr) return head; // 終止條件
        ListNode* ans = myReverse(head->next); // 函式呼叫函示
        head->next->next = head; // 把前面，最倒反過來後的最後面
    ...
    ListNode* addTwoNumbers(ListNode* l1, ListNode* l2) {
        ...
        ListNode* ans = myAddTwoNumbers(list1, list2); // 呼叫上周的 week13-??.cpp
        return myReverse(ans); // 反過來
    }
    // 還缺 myAddTwoNumber()函示 要把它寫出來
    ListNode * myAddTwoNumbers(ListNode* list1, ListNode* list2){
        ...
        if (carry>0) ans2->next = new ListNode(carry); // (差一點) 忘了處理進為囉!
        return ans->next;
    }
};
